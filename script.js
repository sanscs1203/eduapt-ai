...
function pickQuestion(topic, S, usedIds) {
  const pool = QUESTION_BANK[topic] || [];
  const targetDiff = getTargetDifficulty(S);
  const diffOrder = { 'Easy': 0, 'Medium': 1, 'Hard': 2 };

  // Filter out already-used questions, prefer target difficulty
  ...
  // Sort: exact difficulty match first, then closest
  candidates.sort((a, b) => {
...
function updateS(S, wasCorrect) {
  // Update accuracy with exponential moving average
  const alpha = 0.3;
  const newA = +(alpha * (wasCorrect ? 1 : 0) + (1 - alpha) * S.a).toFixed(2);

  // Update frequency (increment each interaction, normalized 0-1 over 20 interactions)
  const newF = Math.min(+(S.f + 0.05).toFixed(2), 1.0);

  // Update difficulty based on new accuracy
  const selfMap = { low: 0.3, mid: 0.6, high: 0.9 };
  const selfVal = selfMap[S.selfLevel] || 0.5;
  const newD = +(0.4 * selfVal + 0.6 * newA).toFixed(2);

  return { ...S, a: newA, f: newF, d: newD };
}
...
if (messagesWrap) {
  ...
  let   pendingQuestion = null; // question currently awaiting student answer
  ...
  function deliverQuestion(topic) {
    const q = pickQuestion(topic, studentS, usedQuestionIds);
    if (!q) {
    ...
    const tierInfo  = getTier(studentS);
...
<span style="font-size:0.82rem;color:var(--text-muted);">Escribe tu respuesta y la compararé con la solución. También puedes escribir <strong>"ver respuesta"</strong> si quieres verla directamente.</span>`;
  ...
  function evaluateAnswer(userText) {
    ...
    if (showDirectly) {
      appendMessage(`La respuesta es:\n\n**${q.answer}**\n\n📚 Recurso de estudio: [${q.id} – ${TOPIC_LABELS[currentTopic]}](${q.link})`, 'bot');
      studentS = updateS(studentS, false);
      persistS();
      updateStatePanelUI();
      setTimeout(() => offerNextQuestion(), 1200);
      return;
    }

    // Simple keyword-based correctness check
    const answerLower = q.answer.toLowerCase();
    const keyTokens   = answerLower.replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter(t => t.length > 1);
    const matchCount  = keyTokens.filter(t => lower.includes(t)).length;
    const isCorrect   = matchCount >= Math.max(1, Math.floor(keyTokens.length * 0.4));

    if (isCorrect) {
      studentS = updateS(studentS, true);
      persistS();
      updateStatePanelUI();
      const tier = getTier(studentS);
      appendMessage(`✅ **¡Correcto!** Muy bien.\n\nRespuesta esperada: **${q.answer}**\n\n📚 Recurso: [${TOPIC_LABELS[currentTopic]}](${q.link})\n\n_Tier actual: ${tier} · S = [${studentS.a}, ${studentS.t}, ${studentS.f}, ${studentS.d}]_`, 'bot');
    } else {
      studentS = updateS(studentS, false);
      persistS();
      updateStatePanelUI();
      appendMessage(`❌ **No exactamente.** Aquí está la solución:\n\n**${q.answer}**\n\n📚 Recurso de estudio: [${TOPIC_LABELS[currentTopic]}](${q.link})\n\n_Revisa el material del link para reforzar este concepto._`, 'bot');
    }

    logInteraction(userText, q, isCorrect);
    setTimeout(() => offerNextQuestion(), 1500);
  }

  function offerNextQuestion() {
    const tier = getTier(studentS);
    appendMessage(`¿Continuamos con **${TOPIC_LABELS[currentTopic]}**? Escribe **"sí"** para otra pregunta, **"cambiar"** para otro tema, o hazme cualquier pregunta sobre álgebra.`, 'bot');
  }
  ...
  function handleMessage(text) {
    ...
    // If there's a pending question, evaluate the answer
    if (pendingQuestion) {
      showTyping();
  ...
  /* ── Topic chips and sidebar topic items ── */
